from networks import EncoderRNN, embed_loss,RDrop
...
class VideoQA():
    ...
    def train(self, epoch):
        ...
        for iter, inputs in enumerate(self.train_loader):
            ...
            logits1, prediction = self.model(video_inputs, qas_inputs, qas_lengths)
            self.model.eval()
            logits2, prediction = self.model(video_inputs, qas_inputs, qas_lengths)
            self.model.train()
            # print(logits1.shape)
            # print(ans_targets.shape)
            
            self.model.zero_grad()
            kl_loss = self.kl_criterion(logits1, logits2, ans_targets, kl_weight=8)
            loss1 = self.criterion(logits1, ans_targets)
            # loss2 = self.criterion(logits2, ans_targets)
    ...
    def predict(self, model_file, result_file):
        ...
        with torch.no_grad():
            for iter, inputs in enumerate(self.val_loader):
                ...
                # 增强图
                self.model.train()
                out1, prediction1 = self.model(video_inputs, qas_inputs, qas_lengths)

                prediction = beta * prediction + (1 - beta) * prediction1
                prediction = prediction.data.cpu().numpy()
